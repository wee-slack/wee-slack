...
def buffer_input_cb(b, buffer, data):
    ...
    if not reaction and not data.startswith('s/') and not data.startswith('s#'):
        ...
        try:
            old, new, global_replace = re.split(r'(?<!\\)/', data)[1:]
        except ValueError:
            pass
        else:
            ...
            # rid of escapes.
            new = new.replace(r'\/', '/')
            sub_count = 1 if global_replace != 'g' else 0
            channel.change_previous_message(old.decode("utf-8"), new.decode("utf-8"), sub_count)
    elif data.startswith('s#'):
        if data.count('#') == 3:
            old, new, global_replace = data.split('#')[1:]
            sub_count = 1 if global_replace != 'g' else 0
            channel.change_previous_message(old.decode("utf-8"), new.decode("utf-8"), sub_count)
...
class Channel(object):
    ...
    def change_previous_message(self, old, new, sub_count=0):
        ...
        if new == "" and old == "":
            ...
            new_message = re.sub(old, new, message["text"], sub_count)
